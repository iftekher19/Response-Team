// server.js
// Response Team - Blood Donation API (cleaned & improved)
...
// Build MONGO_URI more robustly
let MONGO_URI = process.env.MONGO_URI || null;
if (!MONGO_URI && process.env.DB_USER && process.env.DB_PASS && process.env.DB_HOST) {
  // prefer explicit DB_HOST if provided (e.g. cluster0.mongodb.net/myDB)
  // Example: DB_HOST=cluster0.mongodb.net/myDb?retryWrites=true&w=majority
...
if (!MONGO_URI && process.env.DB_USER && process.env.DB_PASS) {
  // older fallback: guess host (not ideal) — user should set MONGO_URI or DB_HOST
  MONGO_URI = `mongodb+srv://${process.env.DB_USER}:${process.env.DB_PASS}@flash0.v0bnf8w.mongodb.net/?appName=flash0`;
}
...
const isValidObjectId = (id) => {
  try {
    // Accept both ObjectId string and ObjectId instance
...
// ---------------- Stripe: create PaymentIntent (existing) ----------------
app.post(
...
app.get(
  ...
  wrap(async (req, res) => {
    ...
    try {
      // retrieve session from Stripe and expand payment_intent
      ...
      return res.send({ ok: true, recorded: true, data: result });
    } catch (err) {
      ...
      switch (event.type) {
        case "payment_intent.succeeded": {
          ...
          try {
            ...
            // idempotency: check existing by stripePaymentIntent
...
// ---------------- Routes: Users ----------------
/**
 * POST  /api/auth/sync  -> accept profile or idToken, upsert user
 * POST  /users          -> upsert user (frontend call after register)
 * GET   /users          -> list / query users
 * PATCH /users/:id      -> partial update (email ignored)
...
// POST /users - upsert by email (called by frontend)
app.post(
  "/users",
  wrap(async (req, res) => {
    const user = req.body || {};
...
app.patch(
  ...
  wrap(async (req, res) => {
    ...
    delete updates.email; // don't allow changing email here
...
// ---------------- Routes: Donation Requests ----------------
/**
 * Canonical routes use /donation-requests
 * Compatibility routes under /requests mirror behavior for older frontend code
 */
...
app.post(
  "/requests",
  wrap(async (req, res) => {
    // simply forward to canonical logic by calling the same handler logic
    // reuse code by calling the /donation-requests handler with same request body
    // but for simplicity duplicate minimal validation
    const payload = req.body || {};
    if (!payload.requesterEmail) return res.status(400).send({ ok: false, message: "requesterEmail is required" });
...
// Get donation requests (canonical)
app.get(
  "/donation-requests",
  wrap(async (req, res) => {
    const { db } = await connectDB();
...
// Compatibility: GET /requests -> list donation-requests (without canonical prefix)
app.get(
  "/requests",
  wrap(async (req, res) => {
    // reuse same logic as /donation-requests
    const { db } = await connectDB();
    const requests = db.collection("donationRequests");
...
// Backwards-compatible GET /requests/my?email=...
app.get(
  "/requests/my",
  wrap(async (req, res) => {
...
// Compatibility: PATCH /requests/:id -> same as donation-requests/:id (status or other)
// Also expose PATCH /requests/:id/status for UI convenience
app.patch(
  "/requests/:id",
  wrap(async (req, res) => {
    // same as canonical update
    const id = req.params.id;
    if (!isValidObjectId(id)) return res.status(400).send({ ok: false, message: "Invalid request id" });
...
// GET /my-donation-requests?email=  (compat)
app.get(
...
app.post(
  ...
  wrap(async (req, res) => {
    const payload = req.body || {};
    if (!payload.userEmail || payload.amount === undefined) return res.status(400).send({ ok: false, message: "userEmail and amount required" });
    ...
    const result = await funds.insertOne(doc);
    res.status(201).send({ ok: true, message: "Fund recorded", data: result });
  })
);
